                    ...
                    >
                            ...
                            <div className="space-y-6">
                                ...
                                {userPosts.length === 0 ? (
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-8">
                                        {userPosts.map(post => (
                                            <div key={post._id} className="bg-slate-900/80 rounded-2xl overflow-hidden relative group border border-white/5 transition-all hover:border-white/10">
                                                <button onClick={() => handleDeletePersonalPost(post._id)} className="absolute top-3 right-3 bg-red-500 hover:bg-red-600 text-white p-2 rounded-xl opacity-0 group-hover:opacity-100 transition-all z-10 shadow-lg" title="Delete vibe"><Trash2 size={16} /></button>
                                                {post.imageUrl && <img src={post.imageUrl} alt="" className="w-full h-40 object-cover" crossOrigin="anonymous" />}
                                                <div className="p-5">
                                                    <span className="bg-primary-500/20 text-primary-300 text-xs font-bold px-3 py-1 rounded-full border border-primary-500/30 mb-3 inline-block">{post.vibe} Vibe</span>
                                            ...
                                            <div key={post._id} className="bg-slate-900/80 rounded-2xl overflow-hidden relative group border border-white/5 transition-all hover:border-white/10">
                                                ...
                                                <div className="p-5">
                                                    <div className="flex justify-between items-center mb-3">
                                                        <span className="bg-primary-500/20 text-primary-300 text-xs font-bold px-3 py-1 rounded-full border border-primary-500/30 inline-block">{post.vibe} Vibe</span>
                                                        <span className="text-slate-500 text-xs">by {post.authorId?.name || 'Unknown'}</span>
                                                    </div>
                                                    <p className="text-white text-base leading-relaxed line-clamp-3">{post.tunedText}</p>
                                                </div>
                                            </div>
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="grid grid-cols-1 gap-4">
                                    {[
                                        ...
                                        <div key={stat.label} className={`bg-slate-900/50 rounded-2xl border ${stat.border} p-6 flex items-center justify-between`}>
                        ...
                        {activeTab === 'admin_users' && isAdmin && (
                            <div className="space-y-6">
                                ...
                                <div className="bg-slate-900/50 rounded-2xl border border-white/5 overflow-hidden">
                                    {allUsers.length === 0 && <div className="text-center py-16 text-slate-500"><Users size={48} className="mx-auto mb-4 opacity-50" /><p>No users found.</p></div>}
                                    <div className="divide-y divide-white/5">
                                        {allUsers.map(u => (
                                            <div key={u._id} className="p-5 flex items-center justify-between hover:bg-white/[0.02] transition-colors">
                                                <div className="flex items-center gap-4">
                                                    {u.picture ? <img src={u.picture} className="w-12 h-12 rounded-full border border-slate-700" referrerPolicy="no-referrer" /> : <div className="w-12 h-12 rounded-full bg-slate-800 border border-slate-700 flex items-center justify-center text-white text-lg font-bold">{u.name?.charAt(0)}</div>}
                                                    <div>
                                                        <p className="text-white font-medium text-base flex items-center gap-2">{u.name}{u.role === 'admin' && <Shield size={14} className="text-accent-400" />}</p>
                        ...
                        {activeTab === 'admin_posts' && isAdmin && (
                            <div className="space-y-6">
                                ...
                                {allPosts.length === 0 && <div className="text-center py-16 bg-slate-900/30 rounded-2xl border border-white/5 text-slate-500"><Shield size={48} className="mx-auto mb-4 opacity-50" /><p>No posts available.</p></div>}
                                <div className="grid grid-cols-1 gap-4">
                                    {allPosts.map(post => (
                                        <div key={post._id} className="bg-slate-900/50 rounded-2xl border border-white/5 p-5 flex gap-5 relative group transition-all hover:border-white/10">
                                            <button onClick={() => handleAdminDeletePost(post._id)} className="absolute top-4 right-4 bg-red-500 hover:bg-red-600 text-white p-2.5 rounded-xl opacity-0 group-hover:opacity-100 transition-all shadow-lg" title="Force delete post"><Trash2 size={16} /></button>
                                            {post.imageUrl && <div className="shrink-0 w-32 h-32 rounded-xl overflow-hidden border border-white/10"><img src={post.imageUrl} className="w-full h-full object-cover" crossOrigin="anonymous" /></div>}
                                            <div className="flex flex-col min-w-0 py-1 pr-12">
                                                <div className="flex items-center gap-2 mb-2">
                                                    <span className="text-sm font-medium text-slate-300">{post.authorId?.name || 'Unknown User'}</span>
                                                    <span className="text-slate-600 text-xs">•</span>
                                                    <span className="text-slate-500 text-xs">{new Date(post.createdAt).toLocaleDateString()}</span>
                                                </div>
                                                <div className="mb-3"><span className="bg-primary-500/20 text-primary-300 text-xs font-bold px-3 py-1 rounded-full border border-primary-500/30 inline-block">{post.vibe}</span></div>
                        ...
                        {activeTab === 'admin_vibe' && isAdmin && (
                            <div className="space-y-6">
                                <div>
                                    ...
                                    <p className="text-slate-400 mt-1">Set a featured style/vibe for the community</p>
                                </div>
                                <form
                                    onSubmit={async (e) => {
                                        e.preventDefault();
                                        if (!vibeName || !description || !weekStart || !weekEnd) { toast.error('Please fill out all fields.'); return; }
                                        setSubmittingVibe(true);
                                        setVibeSuccessMsg('');
                                        try {
                                            await api.post('/admin/weekly-vibe', { vibeName, description, weekStart, weekEnd });
                                            setVibeSuccessMsg('Weekly Vibe scheduled successfully!');
                                            setVibeName(''); setDescription(''); setWeekStart(''); setWeekEnd('');
                                        } catch (err: any) {
                                            toast.error(err?.response?.data?.error || err?.message || 'Failed to schedule weekly vibe');
                                        } finally { setSubmittingVibe(false); }
                                    }}
                                    className="glassmorphism p-6 rounded-3xl border border-white/5 flex flex-col gap-4 max-w-xl"
                                >
                                    {vibeSuccessMsg && <div className="bg-green-500/10 border border-green-500/35 text-green-400 p-4 rounded-2xl text-sm font-semibold">{vibeSuccessMsg}</div>}
                                    <div className="flex flex-col gap-2">
                                        <label className="text-slate-300 text-sm font-medium">Vibe Name</label>
                                        <input type="text" value={vibeName} onChange={(e) => setVibeName(e.target.value)} placeholder="e.g. Ibadan, Old School HipHop" className="bg-slate-900/50 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:border-primary-500 transition-colors" required />
                                    </div>
                                    <div className="flex flex-col gap-2">
                                        <label className="text-slate-300 text-sm font-medium">Description</label>
                                        <textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Describe the mood, origins, and how to write in this style..." className="bg-slate-900/50 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:border-primary-500 resize-none h-28 transition-colors" required />
                                    </div>
                                    <div className="grid grid-cols-2 gap-4">
                                        <div className="flex flex-col gap-2">
                                            <label className="text-slate-300 text-sm font-medium">Week Start</label>
                                            <input type="date" value={weekStart} onChange={(e) => setWeekStart(e.target.value)} className="bg-slate-900/50 border border-white/10 rounded-xl px-4 py-2.5 text-xs text-white focus:outline-none focus:border-primary-500 transition-colors" required />
                                        </div>
                                        <div className="flex flex-col gap-2">
                                            <label className="text-slate-300 text-sm font-medium">Week End</label>
                                            <input type="date" value={weekEnd} onChange={(e) => setWeekEnd(e.target.value)} className="bg-slate-900/50 border border-white/10 rounded-xl px-4 py-2.5 text-xs text-white focus:outline-none focus:border-primary-500 transition-colors" required />
                                        </div>
                                    </div>
                                    <button type="submit" disabled={submittingVibe} className="w-full bg-gradient-to-r from-primary-600 to-accent-600 hover:from-primary-500 hover:to-accent-500 text-white font-bold py-3 px-4 rounded-xl transition-all shadow-lg disabled:opacity-50 mt-2 flex items-center justify-center gap-2">
                        ...
                        {activeTab === 'admin_contract' && (isContractOwner || isPendingContractOwner) && (
                            <div className="space-y-6">
                                <div>
                                    ...
                                    <p className="text-slate-400 mt-1">Manage treasury, validators, and contract state on-chain</p>
                                </div>
                                {isPendingContractOwner && (
                                    <div className="glassmorphism p-6 rounded-3xl border border-green-500/20 bg-green-500/10 flex items-center justify-between">
                                        ...
                                        <button onClick={() => handleContractAction('acceptOwnership')} disabled={contractActionLoading} className="bg-green-600 hover:bg-green-500 text-white font-bold py-3 px-6 rounded-xl transition-colors shadow-lg disabled:opacity-50">Accept Ownership</button>
                                    </div>
                                )}
                                {isContractOwner && (
