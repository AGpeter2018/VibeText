...
const useWritePause = () => {
    ...
    const writePause = useCallback( async () : Promise<boolean> => {
        if (!isConnected) {
        ...
        try {
            loadingToastId = toast.loading('Setting a pause...');
            ...
            if (receipt && receipt.status === 1) {
                ...
                toast.error('Contract pause failed')
                return
            }
            ...
            if (err.message && err.message.includes("execution reverted: You are not the owner")) {
                ...
                toast.error(err.reason || err.message || "Failed to set a contract pause")
            }
        }
